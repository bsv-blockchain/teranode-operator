...
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=blockpersisters/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=blockpersisters/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the BlockPersister object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.16.3/pkg/reconcile
func (r *BlockPersisterReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	result := ctrl.Result{}
	r.Log = log.FromContext(ctx).WithValues("block-persister", req.NamespacedName)
	r.Context = ctx
	r.NamespacedName = req.NamespacedName
	blockPersister := teranodev1alpha1.BlockPersister{}
	if err := r.Get(ctx, req.NamespacedName, &blockPersister); err != nil {
		r.Log.Error(err, "unable to fetch block persister CR")
		return result, nil
	}
