...
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=bootstraps/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=bootstraps/finalizers,verbs=update
...
func (r *BootstrapReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	result := ctrl.Result{}
	r.Log = log.FromContext(ctx).WithValues("bootstrap", req.NamespacedName)
	r.Context = ctx
	...
	statusErr := r.Client.Status().Update(ctx, &bs)
	if err == nil {
		err = statusErr
	}
...
func (r *BootstrapReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&teranodev1alpha1.Bootstrap{}).
