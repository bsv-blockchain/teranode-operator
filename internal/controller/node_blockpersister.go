package controller

import (
	"fmt"
	teranodev1alpha1 "github.com/bitcoin-sv/teranode-operator/api/v1alpha1"
	"github.com/go-logr/logr"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	"sigs.k8s.io/controller-runtime/pkg/controller/controllerutil"
)

// ReconcileBlockPersister is the node blockpersister reconciler
func (r *NodeReconciler) ReconcileBlockPersister(log logr.Logger) (bool, error) {
	node := teranodev1alpha1.Node{}
	if err := r.Get(r.Context, r.NamespacedName, &node); err != nil {
		return false, err
	}
	if !node.Spec.BlockPersister.Enabled {
		return true, nil
	}
	blockPersister := teranodev1alpha1.BlockPersister{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fmt.Sprintf("%s-blockpersister", node.Name),
			Namespace: r.NamespacedName.Namespace,
			Labels:    getAppLabels(),
		},
	}
	_, err := controllerutil.CreateOrUpdate(r.Context, r.Client, &blockPersister, func() error {
		return r.updateBlockPersister(&blockPersister, &node)
	})
	if err != nil {
		return false, err
	}
	return true, nil
}

func (r *NodeReconciler) updateBlockPersister(blockPersister *teranodev1alpha1.BlockPersister, node *teranodev1alpha1.Node) error {
	err := controllerutil.SetControllerReference(node, blockPersister, r.Scheme)
	if err != nil {
		return err
	}
	blockPersister.Spec = *defaultBlockPersisterSpec()

	// if user configures a config map name
	if node.Spec.BlockPersister.Spec != nil {
		blockPersister.Spec = *node.Spec.BlockPersister.Spec
	}
	if node.Spec.Image != "" {
		blockPersister.Spec.Image = node.Spec.Image
	}
	if node.Spec.ConfigMapName != "" {
		blockPersister.Spec.ConfigMapName = node.Spec.ConfigMapName
	}
	return nil
}

func defaultBlockPersisterSpec() *teranodev1alpha1.BlockPersisterSpec {
	return &teranodev1alpha1.BlockPersisterSpec{}
}
