...
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=validators,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=validators/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=validators/finalizers,verbs=update
...
func (r *ValidatorReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	result := ctrl.Result{}
	r.Log = log.FromContext(ctx).WithValues("validator", req.NamespacedName)
	r.Context = ctx
	r.NamespacedName = req.NamespacedName
	validator := teranodev1alpha1.Validator{}
	if err := r.Get(ctx, req.NamespacedName, &validator); err != nil {
		r.Log.Error(err, "unable to fetch validator CR")
		return result, nil
	}
	...
	statusErr := r.Client.Status().Update(ctx, &validator)
	if err == nil {
		err = statusErr
	}
...
func (r *ValidatorReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&teranodev1alpha1.Validator{}).
