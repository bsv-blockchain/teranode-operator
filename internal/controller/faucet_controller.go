...
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=faucets/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=faucets/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the Faucet object against the actual cluster state, and then
...
func (r *FaucetReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	result := ctrl.Result{}
	r.Log = log.FromContext(ctx).WithValues("faucet", req.NamespacedName)
	r.Context = ctx
	r.NamespacedName = req.NamespacedName
	faucet := teranodev1alpha1.Faucet{}
	if err := r.Get(ctx, req.NamespacedName, &faucet); err != nil {
		r.Log.Error(err, "unable to fetch faucet CR")
		return result, nil
	}
	...
	statusErr := r.Client.Status().Update(ctx, &faucet)
	if err == nil {
		err = statusErr
	}
