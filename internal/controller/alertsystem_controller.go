...
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=alertsystems/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=teranode.bsvblockchain.org,resources=alertsystems/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the AlertSystem object against the actual cluster state, and then
...
func (r *AlertSystemReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	as := teranodev1alpha1.AlertSystem{}
	if err := r.Get(ctx, req.NamespacedName, &as); err != nil {
		r.Log.Error(err, "unable to fetch Alert System CR")
		return result, nil
	}
...
func (r *AlertSystemReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&teranodev1alpha1.AlertSystem{}).
